...
- [ ] T001 Instalar dependencias de Tailwind CSS v4 y shadcn/ui en `admin/package.json`
      (`tailwindcss`, `@tailwindcss/vite`, `class-variance-authority`, `clsx`,
      `tailwind-merge`, `lucide-react`) con pnpm
- [ ] T002 [P] Integrar plugin `@tailwindcss/vite` en `admin/vite.config.ts`
- [ ] T003 [P] Configurar `cn()` en `admin/src/lib/utils.ts` (clsx + tailwind-merge) para shadcn/ui
- [ ] T004 [P] Crear el bloque `@import "tailwindcss"` + `@theme` con el Design Token Mapping completo
...
- [ ] T005 [P] Inicializar shadcn/ui con `components.json` y crear los componentes base en
      `admin/src/components/ui/` (button, badge, card, table, dialog, sheet, input, avatar,
      dropdown-menu)
- [ ] T006 [P] Actualizar `admin/src/types/index.ts` con los tipos `Inscripcion`, `Participante`,
      `Tarifa`, `Descuento`, `EstadoInscripcion`, `EstadoDescuento` (reemplaza `admin/src/types.ts` actual)
- [ ] T007 [P] Crear hook `useBreakpoint` en `admin/src/hooks/useBreakpoint.ts` (detección
      responsive desktop/mobile)
- [ ] T008 [P] Crear hook `useAuth` en `admin/src/hooks/useAuth.ts` (sesión actual, `signOut`, loading)
- [ ] T009 [P] Auditar `supabase/migrations/001_inscripciones.sql` y `admin/src/types.ts`
...
- [ ] T010 Crear `supabase/migrations/002_schema_completo.sql` — **renombrar** las columnas
...
- [ ] T013 [P] Definir RLS: `inscripciones` y `participantes` con INSERT-only para rol anon y
      ...
      solo authenticated
- [ ] T014 [P] Crear RPC `obtener_tarifa_vigente()` (SECURITY DEFINER, solo lectura) en
      `supabase/migrations/002_schema_completo.sql` — devuelve la única fila `tarifas.activa`
      vigente por fecha; `LEFT JOIN` con la vista `descuentos_estado` donde
...
- [ ] T015 [P] Crear RPC `buscar_estado_inscripcion()` (SECURITY DEFINER, folio + cédula) en
...
- [ ] T016 [P] [HU3] Implementar `AuthGuard` en `admin/src/auth/AuthGuard.tsx` que redirige a
      `/login` si no hay sesión (FR-011), conectado con `App.tsx`
- [ ] T017 [P] [HU3] Reubicar y refactorizar `admin/src/pages/Login.tsx` → `admin/src/auth/Login.tsx`
      usando componentes shadcn (Input, Button)
- [ ] T018 [P] [HU3] Crear `admin/src/layout/AppLayout.tsx` — layout de dos columnas (sidebar
      izquierdo + área de contenido con `<Outlet/>`), fondo `--fondo-app`, sidebar con
      navegación a Inscripciones/Tarifas
- [ ] T019 [P] [HU3] Crear `admin/src/layout/BottomTabBar.tsx` — bottom tab bar mobile
      (Inscripciones, Tarifas, Perfil)
- [ ] T020 [P] [HU3] Crear hook `useInscripciones` en `admin/src/hooks/useInscripciones.ts`
      (fetch con filtro por estado, search)
- [ ] T021 [HU3] Reubicar y refactorizar `admin/src/components/ListaInscripciones.tsx` →
...
- [ ] T022 [HU3] Crear `admin/src/inscripciones/BuscadorInscripciones.tsx` — input "Buscar por
      folio, cédula o nombre..." (compartido desktop/mobile)
- [ ] T023 [HU3] Crear `admin/src/inscripciones/FiltrosInscripciones.tsx` — chips de filtro por
...
- [ ] T024 [HU3] Crear `admin/src/inscripciones/ExportarExcel.tsx` — botón "Exportar a Excel"
      ...
      monto, estado, fecha (FR-032). Estilo (sólido vs. tint) = Known Gap #1, se ajusta en T041
- [ ] T025 [HU3] Reubicar y refactorizar `admin/src/components/DetalleInscripcion.tsx` →
...
- [ ] T026 [HU3] Modificar `admin/src/App.tsx` — integrar AuthGuard, rutas protegidas con
...
- [ ] T027 [HU4] Implementar lógica `cambiarEstado(id, nuevo_estado, motivo?)` en
...
- [ ] T028 [HU4] Implementar la integración real con Resend en
      ...
      (Principio IV)
- [ ] T029 [HU4] Conectar la Edge Function en `useInscripciones.ts` — invocar
...
- [ ] T030 [HU4] Agregar en `DetalleInscripcion.tsx`: (a) modal de confirmación para "Aprobar"
...
- [ ] T035 [HU5] Validar no-superposición en el cliente dentro de `FormularioDescuento.tsx`
...
- [ ] T036 [HU5] Crear `admin/src/tarifas/FormularioDescuentoDesktop.tsx` — wrapper modal (Dialog
      de shadcn) para desktop, botón "Guardar descuento" deshabilitado en azul pastel hasta
      completar el formulario
- [ ] T037 [HU5] Crear `admin/src/tarifas/FormularioDescuentoMobile.tsx` — pantalla completa con
...
- [ ] T038 [P] [HU5] Crear rutas/pantalla de Tarifas en `App.tsx` o `AppLayout` — integrar
      `TarifaCard` + `ListaDescuentos` + formularios
- [ ] T039 [P] [HU5] Crear `perfil/PerfilSidebar.tsx` (desktop, inline al fondo del sidebar) y
...
- [ ] T040 [P] Verificar Known Gap #2 (color "Programado") — confirmar que `--status-scheduled-*`
      usa `#4B80E8` y NO `--primary`; señalar al propietario si se decide unificar
- [ ] T041 [P] Verificar Known Gap #1 (botón "Exportar a Excel") — resolver con el propietario si
      es sólido azul o tint antes de finalizar `ExportarExcel.tsx`
- [ ] T042 Verificar Known Gap #3 (estados hover/focus) — `opacity` en hover y `box-shadow`
      sutil en focus-visible en botones y chips
- [ ] T043 [P] Ejecutar el build de producción en `admin/` (`pnpm build` → tsc + vite build;
